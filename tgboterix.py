...
async def assistenza_personalizzata_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mostra lo stato del servizio prima di richiedere assistenza
    status_message = get_service_status()
...
async def list_name_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'content_request' in context.user_data:
        # Mostra la tastiera per selezionare il tipo di contenuto
        keyboard = [
...
async def content_details_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Reset del flag
    del context.user_data['content_request']
...
async def admin_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command == 'tickets':
        ...
        if new_status != "operational":
            ...
            # Notifica a tutti gli utenti con ticket aperti
            for ticket_id, ticket in open_tickets.items():
                try:
                    await context.bot.send_message(
        ...
        # Report completo stato servizio
        ...
        if service_status["incident_history"]:
            ...
            for incident in service_status["incident_history"][-5:]:  # Ultimi 5 incidenti
        ...
        """Comando per notificare che un contenuto è stato aggiunto"""
        if len(context.args) < 2:
        ...
        if ticket_id in tickets_db and tickets_db[ticket_id]['type'] == 'content_request':
            # Prepara la notifica per l'utente
            user_msg = (
...
async def public_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mostra lo stato corrente del servizio agli utenti"""
    status_message = get_service_status()
...
def main():
    ...
    # Aggiunta flag per richiesta contenuti
    async def set_content_request_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
        context.user_data['content_request'] = True
        return await richiedi_contenuto_callback(update, context)
    
    # Override per il richiedi contenuto
    application.add_handler(CallbackQueryHandler(set_content_request_flag, pattern='^richiedi_contenuto$'))
    ...
    application.add_handler(CommandHandler('status', admin_commands, filters=filters.Chat(int(ADMIN_CHAT_ID))))
